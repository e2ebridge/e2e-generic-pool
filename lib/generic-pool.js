...
exports.Pool = function (factory) {
  ...
  function scheduleRemoveIdle() {
    if (!removeIdleScheduled) {
      ...
      // removeIdle is called without transaction because it's a timeout
      // todo: we could eventually create the timeout own transaction
   ...
   */
  me.acquire = function (trx, callback, priority) {
  ...
  me.borrow = function (trx, callback, priority) {
    log("borrow() is deprecated. use acquire() instead", 'warn');
    me.acquire(trx, callback, priority);
  };
   ...
   */
  me.destroyAllNow = function(trx, callback) {
